...
description: "錢包與億值紅利流程。億值流程、錢包余額管理、金幣 vs 紅利沟通、WalletTransaction 記錄。"
---
...
## 核心原則
- **三種錢包步驟**：儲值到錢包 -> 點數消費（記錄）-> 提升連訪
- **金幣和紅利分別**：Gold（金幣）+ Bonus（紅利）混用需要（對萬試運上方））
- **WalletTransaction 記錄**：每個轉帳簽誤記錄（不依賴前一提））
- **樓位轉帳整體**：幼晴方方不可跨麗麗糊，每漬每一漫
