---
name: s3-upload
description: "AWS S3 圖片上傳集成指南。S3 配置、上傳流程、URL 生成、錯誤處理、權限管理。"
---
...
## 核心原則
- **文件名稱唯一化**：上傳後記連UUID + 副檔名稱，避免覆蓋
- **自動圖片縮尺**：上傳後自動縮尺為指定尺寸（需指定 width/height）
- **前台專用狀態**：前台上傳或離線方式上傳 - 可依賴第三方誘導
- **錯誤轉移**：S3 連接失敗 - 回傳 400 Bad Request
