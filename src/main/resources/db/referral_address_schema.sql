-- ====================================================
-- 推薦碼系統 & 使用者地址 資料表初始化腳本
-- 執行順序：需在已有 user, store 表之後執行
-- 
-- ⚠️ 重要：請先確認 store 和 user 表的 id 欄位型態
-- 如果是 BIGINT，請先執行 migrate_to_uuid.sql 進行遷移
-- 如果是 VARCHAR(36)，可直接執行本腳本
-- ====================================================

-- 檢查當前表的 id 欄位型態（執行前請手動確認）
-- SELECT TABLE_NAME, COLUMN_NAME, DATA_TYPE, COLUMN_TYPE
-- FROM INFORMATION_SCHEMA.COLUMNS
-- WHERE TABLE_SCHEMA = 'kuji' AND TABLE_NAME IN ('store', 'user') AND COLUMN_NAME = 'id';
...
CREATE TABLE IF NOT EXISTS referral_code (
    ...
    -- 外鍵約束（如果 store 表的 id 是 VARCHAR(36) 才能啟用）
...
CREATE TABLE IF NOT EXISTS referral_record (
    ...
    -- 外鍵約束（如果 user/store 表的 id 是 VARCHAR(36) 才能啟用）
...
CREATE TABLE IF NOT EXISTS user_address (
    ...
    -- 外鍵約束（如果 user 表的 id 是 VARCHAR(36) 才能啟用）
    -- FOREIGN KEY (user_id) REFERENCES user(id) ON DELETE CASCADE
