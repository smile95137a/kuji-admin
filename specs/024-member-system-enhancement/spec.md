...
- Q: 後台審計日誌第一版要覆蓋哪些敏感操作？ → A: 覆蓋會員安全、點數調整、角色/權限、商品/獎品、訂單狀態等高風險異動。
- Q: Admin 手動調整點數時，如何處理併發一致性？ → A: 使用 DB transaction + conditional atomic update，扣除時更新條件包含餘額足夠。
...
前台用戶點擊登出後，目前 session 的 access token 與對應 refresh token 在 server 端立即失效，無法繼續使用；其他裝置 session 不受影響。
...
1. **Given** 用戶已登入並持有有效 token，**When** 呼叫 `POST /api/auth/logout`，**Then** 返回 200，server 端撤銷目前 access token 與對應 refresh token。
...
4. **Given** 同一用戶在其他裝置仍有有效 session，**When** 其中一個裝置登出，**Then** 其他裝置仍可正常使用。
...
後台高風險敏感操作（會員安全、點數調整、角色/權限、商品/獎品、訂單狀態等）均自動記錄操作者、操作內容、修改前後的值。
...
4. **Given** Admin 修改角色權限、商品/獎品關鍵資料或訂單狀態，**When** 操作成功，**Then** 系統產生審計日誌記錄修改前後值。
...
4. **Given** 多個 Admin 或系統流程同時調整同一用戶點數，**When** 任一扣除操作會造成餘額不足，**Then** 該操作 MUST 失敗且不得產生負數餘額或不一致交易記錄。
...
- 同一 Email 或同一 IP 大量請求重送驗證信：僅在上一封驗證信已過 60 秒後才允許重送，回應不得透露帳號是否存在或是否已驗證。
...
- **FR-003**: 系統 MUST 提供公開的重新發送驗證信 API，request body 攜帶 email；API MUST 回傳泛化成功訊息，且同一 email/IP 60 秒內不重複發送。
...
- **FR-011**: 登出後，目前 session 的 access token 與對應 refresh token MUST 立即失效（server-side 撤銷），其他 session MUST NOT 被撤銷。
...
**審計日誌**
- **FR-015**: 後台高風險敏感操作 MUST 自動記錄至 admin_audit_log，包含操作者ID、操作類型、操作對象、修改前後值。
- **FR-016**: 後台 MUST 提供審計日誌查詢 API，支援依操作者、時間範圍、操作類型過濾。
- **FR-016a**: 審計日誌第一版 MUST 覆蓋以下操作類別：會員安全與狀態異動、手動點數調整、角色/權限異動、商品/獎品關鍵資料異動、訂單狀態異動。
...
- **FR-021a**: 點數調整 MUST 在同一 DB transaction 內完成餘額更新、wallet_transaction 記錄與 admin_audit_log 記錄；扣除操作 MUST 使用包含餘額足夠條件的原子更新。

**錢包資料一致性**
- **FR-022**: 系統 MUST 移除對 user_wallet 表的所有讀寫，以 user 表欄位為唯一點數來源。
- **FR-023**: 併發點數調整下，系統 MUST 保證 user 表餘額與 wallet_transaction、admin_audit_log 保持一致；失敗交易不得寫入交易或審計成功記錄。

### Key Entities

- **UserLoginHistory**: 登入歷史記錄，關聯用戶ID，記錄時間/IP/裝置/結果。
- **AdminAuditLog**: 後台操作審計，記錄操作者、目標、操作前後值（JSON）。
- **UserTokenBlacklist**: 前台 token/session 撤銷記錄，用於撤銷指定 access token 與對應 refresh token。
...
- **SC-003**: 前台登出後，目前 session 的舊 token 在 1 秒內完全失效，任何使用該舊 token 的請求返回 401，其他裝置 session 不受影響。
- **SC-004**: 所有登入嘗試（成功、失敗、鎖定）100% 被記錄，無遺漏。
- **SC-005**: 第一版列入範圍的高風險後台敏感操作 100% 生成審計日誌，包含完整的修改前後值。
- **SC-006**: Admin 手動調整點數後，用戶餘額與 wallet_transaction 記錄 100% 一致。
- **SC-007**: 移除 user_wallet 表後，系統功能 100% 正常（無讀寫錯誤）。
- **SC-008**: 併發扣除測試中，任何會導致餘額低於 0 的請求 100% 失敗，且不產生負數餘額或孤立交易記錄。
...
- 前台 token 撤銷採用 per-session/per-token 撤銷策略；不得使用會撤銷該用戶所有 session 的 generation counter 作為一般登出機制。
